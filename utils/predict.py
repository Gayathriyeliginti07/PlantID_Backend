...
# utils/predict.py
# unified transform, top-5 predictions, no duplicate image in each
import os, torch, torch.nn as nn
from torchvision import models, transforms
from PIL import Image
import pandas as pd, base64, io
...
def _gdrive_download(file_id, out_path):
    ...
    if gdown:
        ...
        import gdown as _g
        _g.download(url, out_path, quiet=False)
...
def _load_leaf():
    global leaf_model, leaf_data, leaf_classes
    _gdrive_download(LEAF_FILE_ID, os.path.join(MODEL_DIR, "resnet101_leaf_classifier.pth"))
    leaf_data = pd.read_csv(LEAF_CSV_PATH)
    leaf_data.columns = leaf_data.columns.str.strip().str.lower().str.replace(" ", "_")
    leaf_classes = sorted(leaf_data["label"].unique())
    model = models.resnet101(weights=None)
    model.fc = nn.Linear(model.fc.in_features, len(leaf_classes))
    sd = torch.load(os.path.join(MODEL_DIR, "resnet101_leaf_classifier.pth"), map_location="cpu")
    try:
...
def _top5_predictions(logits, classes, data, label_col):
    probs = torch.softmax(logits, dim=1)[0]
    top5 = torch.topk(probs, 5)
    results = []
    for idx, score in zip(top5.indices.tolist(), top5.values.tolist()):
        cls = classes[idx]
        row = data[data[label_col].str.lower()==cls.lower()].iloc[0]
        results.append({
            "scientific_name": cls,
...
def predict_leaf(image):
    if leaf_model is None: _load_leaf()
    img = image.convert("RGB")
    t = predict_transforms(img).unsqueeze(0).to(device)
    with torch.no_grad(): out = leaf_model(t)
    preds = _top5_predictions(out, leaf_classes, leaf_data, "label")
    return {"predictions": preds, "image": image_to_base64(img)}

def predict_bark(image):
    if bark_model is None: _load_bark()
    img = image.convert("RGB")
    t = predict_transforms(img).unsqueeze(0).to(device)
    with torch.no_grad(): out = bark_model(t)
    preds = _top5_predictions(out, bark_classes, bark_data, "scientific_name")
    return {"predictions": preds, "image": image_to_base64(img)}
